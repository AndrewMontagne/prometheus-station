...
/datum/simple_ui/themed/New(atom/n_datasource, n_width = 512, n_height = 512, n_content_root = "")
	...
	return ..(n_datasource, n_width, n_height, get_asset_datum(/datum/asset/simple/sui_theme_nano))
...
GLOBAL_LIST_EMPTY(sui_template_variables)
GLOBAL_LIST_EMPTY(sui_file_cache)

/datum/simple_ui/themed/proc/get_file(path)
	if(GLOB.sui_file_cache[path])
		return GLOB.sui_file_cache[path]
	else if(fexists(path))
		var/data = file2text(path)
		GLOB.sui_file_cache[path] = data
		return data
	else
