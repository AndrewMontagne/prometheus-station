...
/turf/simulated/floor/engine/attackby(obj/item/weapon/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/weapon/wrench))
		...
		playsound(src.loc, 'cc-by-sa-nc/sound/items/Ratchet.ogg', 80, 1)
		if(do_after(user, 30))
			new /obj/item/weapon/rods(src)
			new /obj/item/weapon/rods(src)
			ReplaceWithFloor()
...
/turf/simulated/floor/attackby(obj/item/weapon/C as obj, mob/user as mob)
	...
	if(istype(C, /obj/item/weapon/crowbar) && intact)
		...
		to_plating()
		playsound(src.loc, 'cc-by-sa-nc/sound/items/Crowbar.ogg', 80, 1)
		...
		if (!src.intact)
			if (C:amount >= 2)
				...
				if(do_after(user, 30))
					...
					playsound(src.loc, 'cc-by-sa-nc/sound/items/Deconstruct.ogg', 80, 1)
			else
