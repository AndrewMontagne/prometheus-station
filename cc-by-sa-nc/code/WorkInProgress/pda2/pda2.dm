...
/obj/item/device/pda2/proc
	...
	toggle_light()
		...
		if (ismob(src.loc))
			if (src.fon)
				src.loc.set_light(src.loc.luminosity + src.f_lum)
			else
				src.loc.set_light(src.loc.luminosity - src.f_lum)
		else
	...
	display_alert(var/alert_message) //Add alert overlay and beep
		if (alert_message)
			playsound(src.loc, 'cc-by-sa-nc/sound/machines/twobeep.ogg', 50, 1)
			for (var/mob/O in hearers(3, src.loc))
				O.show_message(text("\icon[src] *[alert_message]*"))

		src.overlays = null
		src.overlays += image('cc-by-sa-nc/icons/obj/pda.dmi', "pda-r")
		return
