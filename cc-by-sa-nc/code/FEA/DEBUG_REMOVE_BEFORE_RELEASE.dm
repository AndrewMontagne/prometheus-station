	...
	verb
		...
		mark_pipe_networks()
			...
			for(var/obj/machinery/atmospherics/pipe/P in world)
				...
				if(master)
					P.overlays += icon('cc-by-sa-nc/icons/Testing/atmos_testing.dmi',"marker[master.marker]")
				else
					world << "error"
					P.overlays += icon('cc-by-sa-nc/icons/Testing/atmos_testing.dmi',"marker0")

			for(var/obj/machinery/atmospherics/valve/V in world)
				V.overlays = null

				if(V.network_node1)
					V.overlays += icon('cc-by-sa-nc/icons/Testing/atmos_testing.dmi',"marker[V.network_node1.marker]")
				else
					V.overlays += icon('cc-by-sa-nc/icons/Testing/atmos_testing.dmi',"marker0")

				if(V.network_node2)
					V.overlays += icon('cc-by-sa-nc/icons/Testing/atmos_testing.dmi',"marker[V.network_node2.marker]")
				else
					V.overlays += icon('cc-by-sa-nc/icons/Testing/atmos_testing.dmi',"marker0")
		...
		mark_direction()
			...
			for(var/direction in list(NORTH,SOUTH,EAST,WEST))
				if(group_border&direction)
					overlays += icon('cc-by-sa-nc/icons/Testing/turf_analysis.dmi',"red_arrow",direction)
				else if(air_check_directions&direction)
					overlays += icon('cc-by-sa-nc/icons/Testing/turf_analysis.dmi',"arrow",direction)
		air_status()
			set src in world
			set category = "Minor"
...
obj/indicator
	icon = 'cc-by-sa-nc/icons/Testing/air_meter.dmi'
	...
	verb
		...
		mark_groups()
			...
			for(var/turf/simulated/floor/S in world)
				S.icon = 'cc-by-sa-nc/icons/Testing/turf_analysis.dmi'
				if(S.parent)
